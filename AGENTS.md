...
- one commit per patch family, named `retro(<family>): <what>`; a change to a
  family is a fixup folded into its commit;
...
- generated RIR files are never edited by hand.

Build, test and rebase instructions live in the Retrograd workspace, not here:
`AGENTS.md` and `docs/engineering/LLAMA_CPP_FORK_WORKFLOW.md` at the workspace
root. Do not run raw `cmake`/`ctest` invocations to validate a change — use the
lane scripts under `scripts/` there.
