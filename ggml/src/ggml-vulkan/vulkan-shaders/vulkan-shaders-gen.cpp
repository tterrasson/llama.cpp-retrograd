...
// Adding a generated kernel then touches only the
// small translation unit that resolves a blob by name, instead of the 20 000
// lines of ggml-vulkan.cpp — about 6 of the 9 min 30 of a `--build`, for two
// `extern` lines of wiring.
...
void process_shaders() {
    ...
    // beside this line. Both natives are retired; the
    // `rir_` variants below are the only implementation of those two ops.
    // retro delta: RIR-generated variants. Each source is a
    ...
    //
    // The list is generated too, rather than one hand-written line per
    // *variant* here, growing by one every time a kernel gains a lowering, in a
    // vendored file that every rebase has to merge. What a RIR module needs is uniform (no #define, the
