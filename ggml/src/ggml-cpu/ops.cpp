...
// fixed reduction order. Like the GPU kernels, it reuses O from
...
// matters: otherwise a single-sequence ubatch runs the whole op on one thread),
// one group when every v-head shares one q/k head.
