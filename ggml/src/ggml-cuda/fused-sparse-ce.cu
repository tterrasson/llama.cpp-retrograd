...
// retro delta: a position carries n_topk sparse targets
...
static __global__ void fused_sparse_ce_finish_loss(
    ...
    // retro delta: sum_j w_j*(lse - z_j), accumulated in the
...
static __global__ void fused_sparse_ce_make_probs(
    ...
    // retro delta: the softmax half of the gradient is scaled
...
static __global__ void fused_sparse_ce_subtract_target(
    ...
    // retro delta: one subtraction per entry. The gathered
...
// retro delta: with a quantized head the target subtraction cannot index an F32
// copy of the whole head. Each token's target row is captured here, out of the
...
static __global__ void fused_sparse_ce_capture_target_rows(
    ...
    // retro delta: one captured row per entry, so the grid
...
struct fused_sparse_ce_work {
    ...
    // retro delta: one captured logit per entry, [K, n_tokens].
...
// The logits intermediate is then [tile_capacity, seq_chunk] instead of
// [tile_capacity, n_tokens], so its peak does not grow with sequence length.
...
// retro-quant-loader.cuh so no second list of formats appears.
//
    ...
    for (int64_t v0 = 0; v0 < n_vocab; v0 += CE_Q_ROWS) {
        ...
        if (threadIdx.x == 0) {
            for (int r = 0; r < nrows; ++r) {
                ...
                // retro delta: k captured logits per position.
                for (int64_t j = 0; j < n_topk; ++j) {
...
static __global__ void k_fused_sparse_ce_back_decode(
    ...
    // retro delta: grad_h = sum_j coef_j*(acc/Z - w[:,tgt_j]),
...
// place that publishes which regime executed: timings are only comparable
// between two runs that decoded the head the same way, so a measurement must be
// able to say so without reading this file (same rule as the gemm= line of the
// gated delta net backward).
...
void ggml_cuda_fused_sparse_ce_back(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // retro delta: k rows per position, so the capture is
