...
// retro delta: there is no native `ggml_cuda_op_l2_norm_back`.
...
// (RTX 4090, 1.55 µs native / 1.53 µs RIR over
// nine passes). The number is recorded here because it cannot be measured again:
// there is no native kernel left to measure it against.
