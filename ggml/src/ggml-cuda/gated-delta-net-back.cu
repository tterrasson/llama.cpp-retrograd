...
//
// One *thread* per (head, sequence) would be the naive mapping. For a typical
...
// n_tokens * S_v^2 scalar FLOPs with fully uncoalesced access. Such a kernel
...
// it: only one entry state per chunk stays live, so the scratch a per-token
// trajectory would need (1.00 GiB at the sizes this was measured on) is
// divided by C.
//
...
static bool ggml_cuda_gated_delta_net_back_chunked(
    ...
    // The BF16 GEMM is deliberately opt-in: rounding every GEMM input changes the
