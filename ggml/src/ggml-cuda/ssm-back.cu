...
//
// One thread per channel would give every sx read and grad_sx write an
// `ncs` stride between neighbouring threads (one memory transaction each), and
// a Falcon-H1 conv (d_inner+2*n_group*d_state = 896 channels) would fill only
// 14 blocks of 64 threads: 130 us per launch against 6 us for the forward.
// ---------------------------------------------------------------------------
...
//
// One *thread* per (i3, h) would be the naive mapping. At a Mamba2 training
// shape (nc=128, nr=64, nh=24, nt=256) that is 24 threads for the whole GPU,
// each serially walking nt*nr*nc = 2.1M state elements: 742 ms per call.
// ---------------------------------------------------------------------------
